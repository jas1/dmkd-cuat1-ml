...
//			para cada modelo
			for (Entry<String, Map<String,String>> modelo : tipoModelos.getValue().entrySet()) {
				build.append("IF  (").append(modelo.getValue().get(prefixCols[0]) ).append( "   >= " ).append( probabilidadFiltro ).append( " AND clase_int=2 ) " );
...
//				build.append(tabla).append(" where ").append(probab2).append(" >= ").append(probLimiteGanancia).append(" AND ").append(esTrain).append(" = 0").append(UtilidadesGenerales.separator);
	...
	public static String getEnsembleModels(String pathCarpeta,Map<String, Map<String, Map<String,String>>> nombresVariables,String[] prefixCols, String probabilidadFiltro, int calculoGananciaGanancia, int calculoGananciaCosto){
		StringBuilder build = new StringBuilder();
